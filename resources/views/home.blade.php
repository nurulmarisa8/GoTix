        ...
        <form action="{{ route('home') }}#katalog-event" method="GET" class="bg-slate-900/60 backdrop-blur-md p-2 rounded-full border border-white/20 shadow-2xl flex flex-col md:flex-row gap-2 max-w-3xl mx-auto transition hover:bg-slate-900/80">
            <div class="relative flex-1">
                <div class="absolute inset-y-0 start-0 flex items-center ps-4 pointer-events-none">
                    <svg class="w-5 h-5 text-gray-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m19 19-4-4m0-7A7 7 0 1 1 1 8a7 7 0 0 1 14 0Z"/></svg>
                </div>
            ...
            <button type="submit" class="text-white bg-gradient-to-r from-purple-600 to-pink-600 hover:bg-gradient-to-l focus:ring-4 focus:outline-none focus:ring-purple-800 font-bold rounded-full text-sm px-8 py-3 transition transform hover:scale-105 shadow-lg">Cari</button>
        </form>
    </div>

    <div class="absolute bottom-8 left-1/2 transform -translate-x-1/2 animate-bounce z-10">
        <a href="#section-favorit" class="text-white/70 hover:text-white transition flex flex-col items-center gap-2 cursor-pointer">
...
<section id="section-favorit" class="max-w-screen-xl mx-auto px-4 pt-10 pb-10">
    <div class="mb-8 border-b border-pink-900/50 pb-4">
        ...
        <p class="text-slate-400 mt-2">Event yang sudah kamu tandai.</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        @foreach($favorites as $event)
            <div class="bg-slate-900 rounded-2xl overflow-hidden border-2 border-pink-600 shadow-[0_0_20px_rgba(219,39,119,0.3)] hover:-translate-y-2 transition-all duration-300 group flex flex-col h-full relative">
                ...
                <div class="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-pink-500 to-purple-600 z-20"></div>
                ...
                <div class="relative h-56 overflow-hidden flex-shrink-0">
                    ...
                    <div class="absolute top-3 right-3 z-10">
                        <form action="{{ route('event.favorite', $event->id) }}" method="POST">
                            @csrf
                            <button type="submit" class="p-2 rounded-full bg-pink-600 text-white shadow-lg hover:bg-red-600 transition transform hover:scale-110" title="Hapus dari Favorit">
                ...
                <div class="p-6 flex flex-col flex-1 relative bg-gradient-to-b from-slate-900 to-slate-800">
                    <a href="{{ route('event.detail', $event->id) }}" class="mb-auto">
                        <h5 class="mb-2 text-2xl font-bold tracking-tight text-white group-hover:text-pink-400 transition line-clamp-2">
                            {{ $event->name }}
                        </h5>
                    </a>
                    ...
                    <div class="mt-4 space-y-2">
                        <div class="flex items-center text-slate-400 text-sm">
                            <svg class="w-4 h-4 mr-2 text-pink-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
...
{{-- ID ini untuk anchor dari search --}}
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        <div class="bg-slate-800 rounded-2xl overflow-hidden border border-slate-700 shadow-lg hover:shadow-purple-500/20 hover:-translate-y-2 transition-all duration-300 group flex flex-col h-full">
            ...
            <div class="relative h-56 overflow-hidden flex-shrink-0">
                ...
                <span class="absolute top-3 left-3 bg-slate-900/80 backdrop-blur text-white text-xs font-bold px-2 py-1 rounded-md uppercase shadow-lg border border-white/10">
                    {{ $event->category }}
                </span>
                ...
                <div class="absolute top-3 right-3 z-10">
                    @auth
                        <form action="{{ route('event.favorite', $event->id) }}#section-favorit" method="POST">
                            @csrf
                            <button type="submit" class="p-2 rounded-full bg-white/20 backdrop-blur text-white hover:bg-pink-600 shadow-lg transition transform hover:scale-110" title="Tambah ke Favorit">
