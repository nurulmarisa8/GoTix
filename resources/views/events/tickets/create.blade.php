...
@section('title', 'Create Ticket - ' . $event->name)

@section('content')
<div class="max-w-2xl mx-auto px-4 py-8">
    <h1 class="text-3xl font-bold mb-6">Create Ticket for: {{ $event->name }}</h1>

    <form action="{{ route('organizer.events.tickets.store', $event) }}" method="POST" class="bg-white shadow-md rounded-lg p-6">
        @csrf
        ...
        <div class="mb-6">
            ...
            <textarea name="ticket_description" id="ticket_description" rows="3"
                      class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent @error('ticket_description') border-red-300 @enderror">{{ old('ticket_description') }}</textarea>
